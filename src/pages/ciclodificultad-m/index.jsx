...
export default function CicloDificultadM() {
  const DIFICULTAD_M_ID = 9; 
  // Usamos useGetlistaCardsQuery de Redux Toolkit Query
  const { data, error, isLoading } = useGetlistaCardsQuery(DIFICULTAD_M_ID );
  ...
  if (isLoading) {
    return (
      <div className="text-center text-lg mt-10">Cargando Circuitos de Dificultad Intermedia</div>
    );
  }
    ...
    <div>
      <section>
        <ParallaxContainer
          ...
          className=""
          imageUrl="https://www.tucumanturismo.gob.ar/public/img/paseobicigaleria2_wd3gsjrn_16-07-2025.jpg"
        >
          <div className="container mx-auto h-full text-white flex flex-col justify-end">
            <div className="w-11/12 mx-auto pt-5">
              <h2 className="text-5xl md:text-6xl font-bold mb-6">Circuitos de Dificultad Intermedia</h2>
            </div>
          </div>
        </ParallaxContainer>
      </section>
      <div className="w-11/12 mx-auto pt-5">
        <div className="mb-5">
          <Breadcrumb
            items={[{ label: "Circuitos de Dificultad Intermedia", href: "/ciclodificultad-m"}]}
          ></Breadcrumb>
        </div>
        <div className="container mt-5">
          <h1 className="text-center mb-4 text-3xl font-semibold">
            Circuitos de Dificultad Intermedia
          </h1>
        </div>
        <div className="row g-4 mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articulos.length > 0 ? (
            ...
            <div className="col-span-full text-center text-muted">
              No se encontraron Circuitos de Dificultad Intermedia.
            </div>
          )}
        </div>
      </div>
