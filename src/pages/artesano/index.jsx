...
export default function ArtesanosPage() {
  const ARTESANOS_ID = 6; 
  // Usamos useGetlistaCardsQuery de Redux Toolkit Query
  const { data, error, isLoading } = useGetlistaCardsQuery(ARTESANOS_ID);
  ...
  if (isLoading) {
    return (
      <div className="text-center text-lg mt-10">Cargando Ruta del Artesano...</div>
    );
  }
    ...
    <div>
      <section>
        <ParallaxContainer
          ...
          className=""
          imageUrl="https://www.tucumanturismo.gob.ar/public/img/segitinerario4_fpxudcuh_17-07-2025.jpg"
        >
          <div className="container mx-auto h-full text-white flex flex-col justify-end">
            <div className="w-11/12 mx-auto pt-5">
              <h2 className="text-5xl md:text-6xl font-bold mb-6">Ruta del Artesano - Manos del Valle</h2>
            </div>
          </div>
        </ParallaxContainer>
      </section>
      <div className="w-11/12 mx-auto pt-5">
        <div className="mb-5">
          <Breadcrumb
            items={[{ label: "Ruta del Artesano", href: "/artesano" }]}
          ></Breadcrumb>
        </div>
        <div className="container mt-5">
          <h1 className="text-center mb-4 text-3xl font-semibold">
            Ruta del Artesano - Manos del Valle
          </h1>
        </div>
        <div className="row g-4 mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articulos.length > 0 ? (
            ...
            <div className="col-span-full text-center text-muted">
              No se encontró Ruta del Artesano - Manos del Valle.
            </div>
          )}
        </div>
      </div>
